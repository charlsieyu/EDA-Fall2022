...
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() # classic one, white background, gray lines and black axes
print(O3plot2)
...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() # same as bw() but without gridlines
print(O3plot4)
```

Notice that some aspects of your graph have not been adjusted, including: 

* text size (default font size is 11)
* axis label colors
...
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), # axis text to be black from gray
        legend.position = "top") 
  #alternative: legend.position + legend.justification(e.g., right alignment)
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  # horizontal line, lty specify the type of the line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + # alpha: transparency of the points
  # geom_text: add text in the plot, x and y define location, label is the content of text. hjust means horizontal justification, 1 means left/right alignment
  ...
  # date_breaks: break by 2 months instead of 6 months in scaling
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") +
  ylab(expression("O"[3]* " AQI Value")) + # change y axis name, 3 here is subscript

...
huc8_sf_1 <- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map
mapView(huc8_sf, col.regions = "orange", alpha.regions=1) +
  mapView(huc8_sf_1, col.regions = "purple", alpha.regions=1)
# video 3 16'07
```
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% # import column FIPS as factor
...
counties_sf_join <-  merge(x = counties_sf,
                           ...
                           by.x = "GEOID", 
                           by.y = "FIPS" )
# the geometry, the spacial feature always in the end of df
...
wind_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
...
ggplot() + geom_sf(data=huc2_sf,aes(fill=sq_miles))
```
...
# 23 of 100 obs intersect with neuse
...
# add an new area column from the actually intersection above
...
triangle_huc8 <- huc8_sf_utm[triangle_sf,]
mapview(triangle_sf, alpha.regions = 0.3, col.regions='red') +
  mapview(triangle_huc8, alpha.regions = 0)

#Intersect the HUC_8s
triangle_huc8_clip <- huc2_utm %>% 
  st_intersection(triangle_sf)
mapview(triangle_huc8_clip) +
  mapview(triangle_sf, alpha.regions = 0, color='red')
```
...
# Since the point is shown in lat long, we need to first use 4326.
...
resultMask <- st_intersects(counties_sf_utm, 
                            ...
                            sparse = FALSE) 
#The `sparse` option returns a Boolean mask
...
# leaflet cannot work on planar data

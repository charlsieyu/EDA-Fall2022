...
# help build ridge plot which shows the distribution of a variable over time
library(ggridges)
...
# fix some parameters of the plot window
# Scatterplot
...
O3plot2 <- ggplot(EPAair) +
  ...
  # wrong, adding another axis called blue
  # the correct one should be geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) # columns -> year, rows -> site name
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # y axis reverse, the value goes from 0 to 20 from the top to bottom
print(tempvsdepth)

NvsP <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + # the trend line which is blue here 
...
Nutrientplot3 <-
  ...
  # Why didn't we use "fill"? Not clear, just fill the box
print(Nutrientplot3)
...
Nutrientplot4 <-
  ...
  # binaxis = "y": stack observations over y axis
  # stackdir = "center": stack points first on center
  # position = "dodge": observations with same concentration go to the side and not overlay
print(Nutrientplot4)
# In this case, I think dotplot performs maybe the same as the boxplot.
...
print(Nutrientplot6)
# cons: one line for each nutrients, no legend, wrong x axis name
...
print(Nutrientplot7)
# The same plot as the last one but use gather df and don't have those drawbacks
...
print(Nutrientplot6)
# much better than the previous plot, clear to show frequencies of each nutrients

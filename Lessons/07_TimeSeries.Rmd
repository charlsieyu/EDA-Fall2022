...
wind_data <- read.csv(file="./Data/Raw/Wind_Speed_PortArthurTX.csv",header=TRUE,
                      stringsAsFactors = TRUE)
wind_data$DATE <- ym(wind_data$DATE) # ym means year month, use in order, like mdy
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic (can be nonlinear).
...
# origin, seasonal component, trend component, random component
# the larger the right bar, the smaller variance and value in this row
...
* **linear regression**: no seasonality, fits the assumptions of a parametric test. Function: `lm` y~t
* **Mann-Kendall**: no seasonality, non-parametric, missing data allowed. Function: `MannKendall()` (package: Kendall), good for linear trend
* **Seasonal Mann-Kendall**: seasonality, non-parametric `SeasonalMannKendall` (package: Kendall), check the value of each season
* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats), good for linear and nonlinear trend

Specific test for stochastic trend analysis:

* **Augmented Dickey Fuller**: no seasonality, non-parametric, missing data not allowed. Function: `adf.test()` (package: tseries), ADF
...
# the null hypothesis of MK or SMK is the data is stationary

# Inspect results
wind_data_trend1
summary(wind_data_trend1)

# Run SMK test by using another fcn

...
```{r, results = "hide"}
vignette("dplyr")  # Introduction to dplyr
```
...
# useful when there are not only two classes filtered, type less characters
...
NTL.phys.data.TueLake <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth)  # from small to large
...
# add a new column of Fahrenheit degree with column name temperature_F
...
# function month is from package Lubridate
...
# function interval is from package Lubridate
...
NTL.phys.data.processed <- 
  ...
  # when using pipe, we don't need to provide data frame name again in the functions
